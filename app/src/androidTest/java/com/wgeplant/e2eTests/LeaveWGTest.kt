...
class LeaveWGTest {
    ...
    fun setUp() {
        ...
        val uniqueEmail = "${randomNumber}leaveWG@example.com"
    ...
    fun teardown() {
        try {
            composeTestRule.waitUntil(timeoutMillis = 5000) { // only here to fimd out that one error
                composeTestRule.onNodeWithText("Einer WG beitreten").isDisplayed()
            }
            // delete account
    ...
    @Test
    fun testSuccessfulLeaveWGProcess() {
        composeTestRule.waitUntil(timeoutMillis = 10000) {
        ...
        composeTestRule.waitUntil(timeoutMillis = 5000) {
            composeTestRule.onNodeWithText("WG verlassen").isDisplayed()
        }
        composeTestRule.onNodeWithText("WG verlassen").performClick()
        composeTestRule.waitUntil(timeoutMillis = 5000) {
